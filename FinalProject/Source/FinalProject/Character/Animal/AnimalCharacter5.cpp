...
// Sets default values
AAnimalCharacter5::AAnimalCharacter5()
{
	...
	static ConstructorHelpers::FObjectFinder <USkeletalMesh>SkelaetalMesh(TEXT("Skeleton'/Game/InfinityBladeAdversaries/Enemy/Enemy_Bear/Enemy_Bear.Enemy_Bear'"));
	// Set Material Texture
	static ConstructorHelpers::FObjectFinder <UMaterialInterface>Material_(TEXT("Material'/Game/InfinityBladeAdversaries/Enemy/Enemy_Bear/Materials/M_Bear_Master.M_Bear_Master'"));
	//애니메이션 스테이트머신
	static ConstructorHelpers::FObjectFinder<UAnimBlueprint> lAnimBlueprint(TEXT("AnimBlueprint'/Game/InfinityBladeAdversaries/Enemy/Enemy_Bear/BearAnim.BearAnim'"));
	...
	Mesh->SetWorldScale3D(FVector(0.50f, 0.50f, 0.50f));
	CapsuleComponent->SetWorldScale3D(FVector(1.00f, 1.00f, 1.00f));
...
void AAnimalCharacter5::BeginPlay()
{
	Super::BeginPlay();
}

// Called every frame
void AAnimalCharacter5::Tick(float DeltaTime)
{
	...
	if (bDeath == true) {
		...
		aAttackTask* attack = new aAttackTask(this->Controller, AgroTarget, bDamage, 40.0f);
...
void AAnimalCharacter5::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
